...
export class ValidacionService {
  ...
  getMensaje(campo: string): string {
    ...
    if (errorControl) {
      ...
      } else if (errorControl['minlength']) {
        return 'El campo debe contener mínimo ' + errorControl['minlength'].requiredLength + ' caracteres';
      } else if (errorControl['pattern']) {
        if (errorControl['pattern'].requiredPattern === this.numeroPattern) {
          return 'El campo debe ser un número positivo sin comas ni puntos';
        } else if (errorControl['pattern'].requiredPattern === this.decimalPattern) {
          return 'El campo debe ser un número positivo';
        } else if (errorControl['pattern'].requiredPattern === this.correoPattern) {
          return 'El campo debe ser un correo electrónico';
        }
      } else if (errorControl['correoTomado']) {
        return 'Este correo ya se encuentra registrado';
      } else if (errorControl['noExiste']) {
